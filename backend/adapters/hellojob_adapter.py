"""
HelloJob.az Data Adapter
"""
from typing import Dict, Any
from .base_adapter import BaseAdapter, StandardJobPost
from ..nlp.extractor import NLPExtractor


class HelloJobAdapter(BaseAdapter):
    def __init__(self):
        super().__init__("HelloJob.az")
        self.extractor = NLPExtractor()

    def parse_raw_item(self, raw_item: Dict[str, Any]) -> StandardJobPost:
        vac_id = f"hj_{raw_item.get('id', abs(hash(raw_item.get('title', ''))))}"
        title = raw_item.get("title", "Adsız Vəzifə").strip()
        company = raw_item.get("company_name", raw_item.get("company", "Şirkət")).strip()
        sector = raw_item.get("sector", "IT & Rəqəmsal").strip()
        location = raw_item.get("location", "Bakı").strip()
        desc = raw_item.get("description", "").strip()
        min_sal, max_sal = self.extract_salary_range(str(raw_item.get("salary", "")))

        nlp_res = self.extractor.extract_all(f"{title} {desc}")
        return StandardJobPost(
            id=vac_id,
            title=title,
            company=company,
            sector=sector,
            location=location,
            education=nlp_res["education"],
            experience=nlp_res["experience"],
            salary_min=min_sal,
            salary_max=max_sal,
            languages=[l["canonical_name"] for l in nlp_res["languages"]],
            description=desc,
            source="HelloJob.az",
            source_url=raw_item.get("url", "https://hellojob.az"),
            posted_date=raw_item.get("date", ""),
            extracted_skills=nlp_res["technical"] + nlp_res["soft"],
            is_demo=0
        )
